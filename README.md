...
**The scraping takes about 13 hours.** The bottleneck is not the internet connection but the artificial delays — the scraper takes a 2-second break between every request. Without that delay, the code would make hundreds, if not thousands, of requests per minute and potentially crash the website, which amounts to a DDoS attack. Even with the delay, use a VPN during scraping to avoid the school's IP address getting blacklisted.
